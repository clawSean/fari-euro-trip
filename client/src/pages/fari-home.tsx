...
export default function FariHome() {
  ...
  return <main className="min-h-screen bg-[#fffaf5] text-slate-950"><header className="relative overflow-hidden bg-slate-950 text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(244,63,94,0.38),transparent_34%),radial-gradient(circle_at_bottom_left,rgba(251,146,60,0.24),transparent_38%)]" /><div className="relative mx-auto max-w-7xl px-5 pb-16 pt-7 sm:px-8 sm:pb-24"><nav className="flex items-center justify-between"><div className="flex items-center gap-2 font-bold"><span className="text-2xl">🍦</span><span>Euro Summer</span></div><span className="rounded-full border border-white/20 px-3 py-1 text-xs font-bold text-white/70">Fari + Storm · 2026</span></nav><div className="mt-16 max-w-4xl sm:mt-24"><p className="text-sm font-bold uppercase tracking-[0.3em] text-rose-300">London → Florence → coast TBD → Rome</p><h1 className="mt-5 font-serif text-5xl font-bold leading-[0.95] tracking-tight sm:text-7xl lg:text-8xl">Two friends.<br /><span className="text-rose-300">One flexible plot.</span></h1><p className="mt-7 max-w-2xl text-lg leading-8 text-slate-300">A public-safe trip companion for daily inspiration, destination decisions, nightlife, shopping, and the full yacht-era agenda.</p></div><div className="mt-10 flex flex-wrap gap-3"><Pill icon={Plane} copy="Aug 19–31" /><Pill icon={MapPin} copy="Coast decision open" /><Pill icon={Anchor} copy="Boat options welcome" /></div></div></header><div className="sticky top-0 z-40 border-b border-slate-200 bg-[#fffaf5]/95 backdrop-blur"><div className="mx-auto flex max-w-7xl gap-2 overflow-x-auto px-4 py-3 sm:px-8">{tabs.map(([id, label, Icon]) => <button key={id} onClick={() => setTab(id)} className={`inline-flex shrink-0 items-center gap-2 rounded-full px-4 py-2 text-sm font-bold ${tab === id ? "bg-slate-950 text-white" : "bg-white text-slate-600"}`}><Icon className="h-4 w-4" />{label}</button>)}</div></div><div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 sm:py-20">{tab === "route" && <RouteTab />}{tab === "choose" && <ChooseTab />}{tab === "spotlights" && <SpotlightsTab />}{tab === "plan" && <PlanTab />}</div><footer className="border-t bg-white"><div className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-8 text-sm text-slate-500 sm:flex-row sm:justify-between sm:px-8"><p>Built for flexible plans and excellent outfits. 🍦</p><p className="inline-flex items-center gap-2"><LockKeyhole className="h-4 w-4" /> Exact trip details stay private.</p></div></footer></main>;
}
